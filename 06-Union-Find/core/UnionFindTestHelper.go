...
// 路径压缩
func (this *UnionFindTestHelper)TestUF5(n int)   {
	uf := NewUnionFind5(n)
	...
	// 打印输出对这2n个操作的耗时
	fmt.Printf("UF5 Path-Compression 2*n=%d,took %f%s",2*n, elapsed.Seconds(), "s \n")

}
// 路径压缩-- 递归
func (this *UnionFindTestHelper)TestUF6(n int)   {
	uf := NewUnionFind6(n)
	...
	// 打印输出对这2n个操作的耗时
	fmt.Printf("UF6 Path-Compression-recursion 2*n=%d,took %f%s 递归消耗不少时间 \n",2*n, elapsed.Seconds(), "s")
